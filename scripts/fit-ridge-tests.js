// Fit log(dps) = b0 + b1*ilvl + b2*lvlReq + b3*rarity + b4*hand using ridge regression
const endpoint = 'https://production-api.waremu.com/graphql/'
const ids = ['419300','419325','419061','419036','418797','418772','3417','3419']
const RAPIER = new Set(['419300','419036','418772'])
const CLAY = new Set(['419325','419061','418797','3419'])
...
function ridgeSolve(X, y, lambda){
  ...
  for (let i=0;i<n;i++){
    for (let a=0;a<p;a++){
      XT_y[a] += X[i][a]*y[i]
      for (let b=0;b<p;b++) XT_X[a][b] += X[i][a]*X[i][b]
    }
  }
  // add lambda to diagonal
  for (let i=0;i<p;i++) XT_X[i][i] += lambda
  // solve linear system XT_X * beta = XT_y via gaussian elimination
  const A = XT_X.map(r=>r.slice()); const B = XT_y.slice()
  for (let i=0;i<p;i++){
    let piv = i
    for (let j=i;j<p;j++) if (Math.abs(A[j][i]) > Math.abs(A[piv][i])) piv = j
    if (Math.abs(A[piv][i]) < 1e-15) return null
...
(async ()=>{
  ...
  for (const id of ids){
    ...
    const hand = RAPIER.has(id) ? 0 : (CLAY.has(id) ? 1 : 0)
  ...
  if (!beta) { console.error('ridge solve failed'); return }
  console.log('Ridge-fitted coefficients (log dps):')
  ...
  const fs = require('fs'); fs.writeFileSync('tmp/ridge-coefs.json', JSON.stringify({ intercept: beta[0], ilvl: beta[1], lvlReq: beta[2], rarity: beta[3], hand: beta[4] }, null, 2))
  console.log('Wrote tmp/ridge-coefs.json')
