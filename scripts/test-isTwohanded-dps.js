// Test two items using DPS-only deterministic classifier
...
(async ()=>{
  try {
    for (const id of ids){
      ...
      if (d==null){ console.log('No dps for', id); continue }
      const res = classifyDps(d, it)
      console.log(`\nItem ${id}: ${it.name} (slot=${it.slot}, type=${it.type}, ilvl=${it.itemLevel}, lvlReq=${it.levelRequirement}, rarity=${it.rarity})`)
      console.log(` observed dps=${d}, predicted 1H=${res.p1.toFixed(2)}, predicted 2H=${res.p2.toFixed(2)}`)
