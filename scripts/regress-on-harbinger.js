/*
  Fit log(dps) for Harbinger family items and test predictions using the fitted coefficients.
  Model: log(dps) = b0 + b1*ilvl + b2*levelRequirement + b3*rarity + b4*renown + b5*hand
  Usage: node scripts/regress-on-harbinger.js
*/
const fs = require('fs')
...
(async ()=>{
  console.log('Starting Harbinger-specific DPS regression and test...')
  ...
  fs.writeFileSync(path.resolve(__dirname,'..','tmp','rows-harbinger.json'), JSON.stringify(rows, null, 2))
  ...
  console.log('\nCoefficients (log dps) for Harbinger:')
  ...
  fs.writeFileSync(path.resolve(__dirname,'..','tmp','coefs-harbinger.json'), JSON.stringify({ intercept: beta[0], ilvl: beta[1], lvlReq: beta[2], rarity: beta[3], renown: beta[4], hand: beta[5], r2 }, null, 2))
  console.log('Wrote tmp/coefs-harbinger.json')

  // Classification summary using the fitted coefficients
  const marginLog = 0.223
  let counts = { total: n, two_high:0, two_low:0, one_high:0, one_low:0 }
  const mis = []
  for(let r of rows){ const obs = r.d; const lobs = Math.log(obs); const p1 = Math.exp(beta[0] + beta[1]*r.ilvl + beta[2]*r.lvlReq + beta[3]*r.rf + beta[4]*r.ren + beta[5]*0); const p2 = Math.exp(beta[0] + beta[1]*r.ilvl + beta[2]*r.lvlReq + beta[3]*r.rf + beta[4]*r.ren + beta[5]*1); const e1 = Math.abs(lobs - Math.log(p1)); const e2 = Math.abs(lobs - Math.log(p2)); const logDiff = e1 - e2; let isTwo=false; let conf='low'; if (logDiff > marginLog){ isTwo=true; conf='high' } else if (logDiff < -marginLog){ isTwo=false; conf='high' } else { isTwo = (logDiff>0); conf='low' } if (isTwo && conf==='high') counts.two_high++; else if (isTwo) counts.two_low++; else if (!isTwo && conf==='high') counts.one_high++; else counts.one_low++; const base = r.speed ? (r.d * (r.speed/1000)) : null; mis.push({ id: r.id, name: r.name, observedDps: r.d, observedBase: base, pred1: p1, pred2: p2, isTwo, conf, logDiff }) }

  console.log('\nClassification summary (Harbinger rows):', counts)
  console.log('\nTop 10 rows by absolute log error (for observed vs chosen prediction):')
  ...
  for(let i=0;i<Math.min(10, mis.length); i++){ const m=mis[i]; console.log(`${m.id} | ${m.name} | obsDps=${m.observedDps} pred1=${m.pred1.toFixed(1)} pred2=${m.pred2.toFixed(1)} -> isTwo=${m.isTwo} conf=${m.conf} logDiff=${m.logDiff.toFixed(3)}`) }
